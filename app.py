...
with col2:
    st.markdown("""
        <div style="display:flex; align-items:center; justify-content:center; gap:10px;">
          <div class="main-title" style="margin:0;">Carboneras de Guadazaón</div>
          <img src="https://raw.githubusercontent.com/jorgeargudoo/RecomendadorTuristicoInteligente/main/imagenes/escudo.png"
               alt="Escudo de Carboneras de Guadazaón"
               style="height:90px; width:auto; border-radius:6px;">
        </div>
        <div class="subtitle">DONDE REPOSA EL SUEÑO DEL NUEVO MUNDO</div>
        """, unsafe_allow_html=True)
...
if st.session_state.stage == "form":
    with st.form("form_recomendador", clear_on_submit=False):
        ...
        <div class="info-card">
          ...
          <ul>
            <li><b>Edad y género</b> orientan el tono de las propuestas.</li>
            <li><b>Si vives aquí</b> prioriza rincones menos obvios o esenciales si vienes de fuera.</li>
            <li><b>Frecuencia</b> calibra cuánto explorar vs. ir a tiro fijo.</li>
            <li><b>Tipos de actividades por público</b> nos dicen qué recomendarías.</li>
          </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.expander("ℹ️ Cómo funciona en 10 segundos"):
            st.markdown("""
            - Calculamos tus **preferencias** (modelo multi‑salida).
            - Aplicamos **filtro meteorológico** con lógica difusa (AEMET + UV).
            - Verás **recomendados** y puedes alternar a **Puntos de Interés**.
            """)
...
if st.session_state.stage == "compute":
    with st.spinner("💡 Pensando tus recomendaciones..."):
        ...
        recomendaciones = {l: int(p) for l,p in zip(lugares, pred)}

        try:
            clima = obtener_clima_hoy()
            score = recomendar(clima)
            recomendaciones = filtrar_por_clima(recomendaciones, clima, score)
            st.session_state.clima_hoy = clima
            st.session_state.score_exterior = score
            st.info(f"Filtrado climático aplicado. Score exterior: {score:.2f}")
        except Exception as e:
            st.session_state.clima_hoy = None
            st.warning("No se pudo obtener el clima actual. Las recomendaciones no han sido filtradas.")
            st.text(f"Error: {e}")

        st.session_state.lugares_recomendados = [k for k,v in recomendaciones.items() if v==1]
...
if st.session_state.stage == "results":
    ...
    st.markdown(f"### {'Puntos de Interés' if mostrar_todos else 'Recomendaciones para ti'}")

    if mostrar_todos:
        mostrar_mapa_recomendaciones(LUGARES_INFO, LUGARES_INFO, map_key="mapa_todos")
    else:
        recs = st.session_state.get("lugares_recomendados", [])
        if recs:
            mostrar_mapa_recomendaciones(recs, LUGARES_INFO, map_key="mapa_recomendados")
        else:
    ...
    etiqueta = "Volver a ver tus recomendaciones" if mostrar_todos else "Mostrar todos los puntos de interés"
    if st.button(etiqueta, key="btn_toggle_mapa"):
        log_event("toggle_show_all", {
            "user_id": st.session_state.user_id,
            "from": "all" if mostrar_todos else "recommended",
            "to": "recommended" if mostrar_todos else "all"
        })
        st.session_state.mostrar_todos = not mostrar_todos

    feedback = st.slider(
        "¿Qué valoración darías a estas recomendaciones?",
        1, 5, st.session_state.feedback,
        key="feedback_slider",
        disabled=st.session_state.valoracion_enviada
    )
    st.session_state.feedback = feedback
    st.write("Tu valoración:", "⭐" * int(feedback))
